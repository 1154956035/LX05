...
namespace DAL
{
    public class Activity
    {
        ...
        public IEnumerable<Model.Activity> GetVerityPage(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
        ...
        public IEnumerable<Model.Activity> GetNew()
        {
            using (IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "select * from activity where activityVerify='审核通过' and endTime>=now() order by endTime edsc limit 2";
        ...
        public int UpdateImg(Model.Activity active)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "usdate activity set activitypicture=@activityPicture where activityid=@acticityId";
                return cn.Execute(sql,active);
            }
        }
